...
def test(model, loader, threshold=0.5):
    ...
    with torch.no_grad():
        ...
        for img,label in loader:
            ...
            y_pred.extend(pre.sigmoid().flatten().tolist())
...
if __name__ == '__main__':
    ...
    # 去掉扩展名（假设扩展名是从文件名末尾的最后一个点开始）
    output_df['img_name'] = output_df['img_name'].str.replace(r'\.[^.]*$', '', regex=True)

    # 按照字典序进行排序，区分大小写
    output_df = output_df.sort_values(by='img_name', ascending=True)
    output_df.to_csv(f'./{file_name}', index=False, header=False)
    print("运行完成")
