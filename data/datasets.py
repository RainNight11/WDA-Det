...
class RealFakeDataset(Dataset):
    def __init__(self, opt):
        ...
        #assert opt.data_mode in ["ours", "wang2020", "ours_wang2020","RFNT"]
        ...
        if opt.data_mode == 'ours':
            ...
            temp = 'train/progan' if opt.data_label == 'train' else 'test/progan'  # train/progan
            ...
            if opt.attack_aug:
                attack_aug_root = opt.wang2020_data_path + "_adv"
                real_list += get_list(os.path.join(attack_aug_root, temp), must_contain='0_real')
                fake_list += get_list(os.path.join(attack_aug_root, temp), must_contain='1_fake')
            ...
            datasets_list = ['samples']  # kagglephase1 太大
            dataset_path = "./datasets/"
            temp = 'train' if opt.data_label == 'train' else 'valid'
            real_list = []
            fake_list = []
            for dataset_index in datasets_list:
                real_list += get_list(os.path.join(dataset_path,dataset_index,temp,"real"))
                fake_list += get_list(os.path.join(dataset_path,dataset_index,temp,"fake"))

        elif opt.data_mode == 'WildRF':
        ...
        # 仅使用 1/10 的数据
        subset_len = len(self.total_list)

        self.total_list = self.total_list[:subset_len]
        if opt.isTrain:
